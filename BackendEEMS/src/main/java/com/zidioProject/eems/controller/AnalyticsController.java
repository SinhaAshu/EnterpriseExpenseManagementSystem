...
public class AnalyticsController {
    ...
    @GetMapping("/monthly")
    public ResponseEntity<List<ExpenseStatsDTO>> getMonthlyTrends() {
    ...
    @PreAuthorize("hasRole('Admin')")
    @GetMapping("/category")
    public ResponseEntity<List<ExpenseStatsDTO>> getCategoryBreakdown() {
    ...
    public ResponseEntity<byte[]> exportExcel() {
        byte[] data = analyticsService.exportToExcel();
    ...
    public ResponseEntity<byte[]> exportPdf() {
        byte[] data = analyticsService.exportToPDF();
        return ResponseEntity.ok()
