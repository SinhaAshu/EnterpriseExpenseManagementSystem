...
const ManagerExpenseRequests = () => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        const response = await axios.get('http://localhost:8080/api/manager/expense/requests', {
          headers: {
  ...
  const handleStatusUpdate = async (id, status) => {
    ...
    try {
      ...
      window.alert(`Error updating expense: ${error.response?.data?.message || error.message}`);
...
export default ManagerExpenseRequests
